...
void *theoraStreamReader(void *arg) {
    ...
    struct threadParam1 *a = arg;
    char *filename = (char *) (*a).nomFichier;
	...
	if (respac == 0) {
	    ...
	    s = getStreamState(& oggtheorastate, & theorapage,TYPE_THEORA,&(*a).m);
	...
	if ( decodeAllHeaders(respac, s, TYPE_THEORA, &(*a).m ))
	    continue;

...
struct streamstate *getStreamState(ogg_sync_state *pstate, ogg_page *ppage,
				   enum streamtype type, pthread_mutex_t *mutex) {
  ...
  //printf("[S_C1]Devant mutex\n");
  pthread_mutex_lock(mutex);
	if (type == TYPE_THEORA)
	    HASH_ADD_INT( theorastrstate, serial, s );
	else
	    HASH_ADD_INT( vorbisstrstate, serial, s );
    //  printf("[S_C1]Entrée mutex\n");
  pthread_mutex_unlock(mutex);
//  printf("[S_C1]Sortie mutex\n");
    } else {
	// proteger l'accès à la hashmap
//  printf("[S_C2]Devant mutex\n");
  pthread_mutex_lock(mutex);
	if (type == TYPE_THEORA)
	    HASH_FIND_INT( theorastrstate, & serial, s );
	else
	    HASH_FIND_INT( vorbisstrstate, & serial, s );
  //    printf("[S_C2]Entrée mutex\n");
  pthread_mutex_unlock(mutex);
  //  printf("[S_C2]Sortie mutex\n");
...
int decodeAllHeaders(int respac, struct streamstate *s, enum streamtype type,
	...
	if (res != TH_ENOTFORMAT) {
        ...
        pthread_t tid;
        struct threadParam2 param;
        param.serial = s->serial;
        param.m = *mutex;
        pthread_create(&tid,NULL,draw2SDL,&param);
